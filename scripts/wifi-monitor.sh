...
while true; do
  ...
  ping_result=$(ping -c 1 -W 2 10.0.0.10 2>/dev/null && echo "OK" || echo "FAIL")
  ping_status=$(echo "$ping_result" | tail -1)
  ...
  echo "${timestamp} | signal=${signal:-?}dBm | tx=${tx_rate:-?} | rx=${rx_rate:-?} | ping=${ping_status} | stream_age=${stream_age}s | restarts=${restarts} | ffmpeg=${ffmpeg_pid}" >> "$LOG"
